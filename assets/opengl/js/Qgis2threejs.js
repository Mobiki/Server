...
Q3D.Options = {
  ...
  qmarker: {r: 0.25, c: 0x000000, o: 0.8},
  debugMode: false,
  exportMode: false,
...
Q3D.application = {
  ...
  loadProject: function (project) {
    ...
    if (Q3D.Controls) {
      ...
      this.controls.maxPolarAngle = 0.95 * Math.PI / 2;
    ...
    var _m = new THREE.SpriteMaterial({color: 0xffffff, map :  THREE.ImageUtils.loadTexture('/images/icon/marker.png'), fog: true });
    _m.map.minFilter = THREE.LinearFilter;
    _m.useScreenCoordinates = false;
    this.queryMarker = new THREE.Sprite(_m);

    this.queryMarker.scale.set(372 / 128, 594 / 128, 1);

    this.queryMarker.visible = false;
    this.scene.add(this.queryMarker);

    this.highlightMaterial = new THREE.MeshLambertMaterial({color: 0xDB6900, transparent: false, opacity: 1});
  ...
  eventListener: {
    resize: function () { 
      ...
      if( parseInt($('.panel-room-info').css('height')) == 65){
        this.setCanvasSize(window.innerWidth, window.innerHeight - parseInt($('.panel-room-info').css('height')));
      }
      else
  ...
  buildDefaultLights: function (parent) {
    ...
    // ambient light
    //parent.add(new THREE.AmbientLight(0xDDD));
    parent.add(new THREE.AmbientLight(0xbbbbbb));
    ...
    var light1 = new THREE.DirectionalLight(0xc0c0d0, 0.3);
    light1.position.set(x, y, z);
    light1.castShadow = true;
    light1.shadowCameraNear = true;
    parent.add(light1);

    // thin light from the opposite direction
    var light2 = new THREE.DirectionalLight(0xaabbff, 0.1);
    light2.position.set(-x, -y, -z);
    parent.add(light2); 
  },

  buildDefaultCamera: function () {
    this.camera = new THREE.PerspectiveCamera(15, this.width / this.height, 0.1, 1000);
    this.camera.position.set(130, -100, 100);
  },
  ...
  highlightFeature: function (layerId, featureId, auto_removed) {
    if (this.highlightObject && !auto_removed) {
    ...
    if(auto_removed && this.highlightObject)
  ...
  canvasClicked: function (e) {
    ...
    for (var i = 0, l = objs.length; i < l; i++) {
      ...
      if (layerId !== undefined && featureId !== undefined && layer.a !== undefined) {
        ...
        if(
          typeof f.a[0]['id'] !== 'undefined'
          && typeof f.a[0]['feature_id'] !== 'undefined' && f.a[0]['feature_id'].length > 0
        ){

          this.queryMarker.position.set(f.pts[0][0], f.pts[0][1], f.pts[0][2] + 2.1);
          this.queryMarker.visible = true;

          for(var i=0; i<f.a[0]['feature_id'].length; i++){
              this.highlightFeature((layerId === undefined) ? null : layerId - 1, //layer -1 => polygon layers
                (f.a[0]['feature_id'][i] === undefined) ? null : f.a[0]['feature_id'][i], true); //true => autor
          }

            var _r_id = parseInt(f.a[0]['id']);
          $.ajax({
              method  : 'POST',
              url     : '/room/details/'+_r_id,
              dataType: 'json',
              data    : { },
        ...
        }else if(typeof f.a[0]['type'] !== 'undefined' && f.a[0]['type'] == 'room' && typeof f.a[0]['id'] !== 'undefined' && parseInt(f.a[0]['id']) > 0){

          for(var i=0; i < layer.f.length; i++){
            if(
              typeof layer.f[i].a[0]['id'] !== 'undefined'
              && layer.f[i].a[0]['id'] === f.a[0]['id']
            ){
              this.highlightFeature((layerId === undefined) ? null : layerId, //layerId => polygon layers
                  (layer.f[i].objs[0]['userData']['featureId'] === undefined) ? null : layer.f[i].objs[0]['userData']['featureId'], true); //true => autor
            }
          }

          var layer_2 = this.project.layers[layerId + 1]; //layer + 1 => text layer
          for(var i=0; i<layer_2.f.length; i++){
            if(
              typeof layer_2.f[i].a[0]['id'] !== 'undefined'
              && layer_2.f[i].a[0]['id'] === f.a[0]['id']
            ){
              this.queryMarker.position.set(layer_2.f[i].pts[0][0], layer_2.f[i].pts[0][1], layer_2.f[i].pts[0][2] + 2.1);
              this.queryMarker.visible = true;
            }
          }          
          ...
          Q3D.application.setCanvasSize(window.innerWidth, window.innerHeight - parseInt($('.panel-room-info').css('height')));
        }
      }
...
Q3D.PointLayer.prototype.build = function (parent) {
  ...
  if (this.objType == "Text") { this.buildTexts(parent); return; }
...
Q3D.PointLayer.prototype.buildIcons = function (parent) {
  ...
  this.f.forEach(function (f, fid) {
    ...
    var image = this.project.images[f.m];
...
Q3D.PointLayer.prototype.buildTexts = function (parent) {
  // each feature in this layer
  this.f.forEach(function (f, fid) {
    var mat = this.materials[f.m];

    var scale = (f.scale === undefined) ? 1 : f.scale;
    var size = this.getSizeOfText(f.n);
    var sx = size.width / 2 * scale,
        sy = size.width / 2 * scale;
...
Q3D.PointLayer.prototype.getSizeOfText = function (txt){
  // Create a dummy canvas (render invisible with css)
  var c=document.createElement('canvas');
  // Get the context of the dummy canvas
  var ctx=c.getContext('2d');
  // Set the context.font to the font that you are using
  ctx.font = Q3D.Options.label.minFontSize + 'px' + Q3D.Options.label.fontFamily;

  return { width : ctx.measureText(txt).width, height : Q3D.Options.label.minFontSize * 1.4 };
};
  ...
  if (this.objType == "Line") {
    var createObject = function (f, line) {
      ...
      for (var i = 0, l = line.length; i < l; i++) {
        pt = line[i];
        geom.vertices.push(new THREE.Vector3(pt[0], pt[1], ((typeof pt[2] !== 'undefined') ? pt[2] : f.zs)));
      }
