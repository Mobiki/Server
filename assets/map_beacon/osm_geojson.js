    ...
    function togeojson(geo, properties) {
        ...
        switch (geo.type) {
            case 'Point':
                ...
                '" lon="' + geo.coordinates[0] + '" action="modify" visible="true">';
            ...
            case 'LineString':
                //Burası
                ways += '<way id="' + count + '" action="modify" visible="true">';
                count--;

                var _ids = [];
                for (var i = 0; i < geo.coordinates.length; i++){
                    _ids.push(count);
                    nodes += '<node id="' + count + '" lat="' + geo.coordinates[i][1] + '" lon="' + geo.coordinates[i][0] + '" action="modify" visible="true" />';
                    count--;
                }
            ...
            /*case 'MultiPolygon':
                relations += '<relation id="' + count + '" action="modify" visible="true">';
    ...
    function polygon(geo, properties, multipolygon) {
        ...
        if (geo.coordinates.length > 1) {
            ...
            for (var i = 0; i < geo.coordinates.length; i++) {
                ...
                if (multipolygon) relations += '<member type="way" ref="' + count + '" role="' + role + '"/>';
